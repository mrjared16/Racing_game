...
list <Obstacle> list_vc;		//dslk dùng để lưu các vật cản

int dan;	//so vien dan co the ban
list <Bullet> list_dan;		//nhung vien dan da duoc ban ra
...
void gameOver() {
	...
	showBackButton(8, 8);	//Back to main menu
...
void pause()
{
	...
	int n_label = 2;
	MenuFuntions(label, n_label, index, LIGHT_RED, CHIEU_RONG + 1, CHIEU_DAI / 2);
...
bool updateTrangThai()
{
	...
	if (!updateObstacles(list_vc, car, stats))		//=> updateObstacles false khi vat can va cham vao xe 
...
void show()
{
	...
	//ve vat can
	drawObstaclesOnBuffer(list_vc, map, RED);
	...
	const char *content[] = { "LEVEL %d", "Score: %3d", "Bullet: %3d" };
	int stat[] = { stats.level, stats.diem, dan};
	int n = 3;
...
void play(bool auto_play)
{
	khoiTao();
	system("cls");
	PlaySound(TEXT("123.wav"), NULL, SND_FILENAME | SND_LOOP | SND_ASYNC);
	while (1) {
